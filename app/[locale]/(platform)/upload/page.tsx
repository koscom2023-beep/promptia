...
export default function UploadPage() {
  ...
  const handleNovelSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 자동 모더레이션 (비동기, 실패해도 업로드는 성공)
      try {
        const { autoModerateNovel } = await import("@/app/actions/moderation");
        await autoModerateNovel(
          novel.id,
          novelTitle,
          novelDescription,
          novelContent
        ...
        // 모더레이션 실패는 무시 (업로드는 성공)
        console.warn("자동 모더레이션 실패:", moderationError);
      }
    ...
    try {
      ...
      try {
        ...
        await autoModerateNovel(
          ...
          webtoonTitle,
          webtoonDescription,
          webtoonImageUrls // 이미지 URL 문자열을 콘텐츠로 사용
        );
      } catch (moderationError) {
        // 모더레이션 실패는 무시 (업로드는 성공)
        console.warn("자동 모더레이션 실패:", moderationError);
    ...
    try {
      ...
      try {
        ...
        await autoModerateNovel(
          ...
          videoTitle,
          videoDescription,
          videoYoutubeUrl // 유튜브 URL을 콘텐츠로 사용
        );
      } catch (moderationError) {
        // 모더레이션 실패는 무시 (업로드는 성공)
        console.warn("자동 모더레이션 실패:", moderationError);
